...
public:
    ...
    void init();
    void setServerInformationForStudy(ctkDICOMRetrieve* retrieve,QString studytUID);
...
//----------------------------------------------------------------------------
void ctkDICOMQueryRetrieveWidgetPrivate::setServerInformationForStudy(ctkDICOMRetrieve* retrieve, QString studytUID)
{
    Q_Q(ctkDICOMQueryRetrieveWidget);
    // Get information which server we want to get the study from and prepare request accordingly
    ctkDICOMQuery *query = this->QueriesByStudyUID[studytUID];
    retrieve->setDatabase( this->RetrieveDatabase );
...
void ctkDICOMQueryRetrieveWidget::retrieve()
{
    ...
    QModelIndex currentIndex = d->results->currentIndex();
    QString currentUID = d->Model.data(currentIndex,ctkDICOMModel::UIDRole).toString();
    int currentType = d->Model.data(currentIndex,ctkDICOMModel::TypeRole).toInt();

    if(currentType==static_cast<int>(ctkDICOMModel::SeriesType))
    {
        logger.debug("series");
        QString studyUID = d->Model.data(currentIndex.parent(),ctkDICOMModel::UIDRole).toString();

        d->setServerInformationForStudy(retrieve, studyUID);
        if(d->UseProgressDialog){
        ...
        try
        {
            if (d->QueriesByStudyUID[studyUID]->preferCGET())
            {
                retrieve->getSeries ( studyUID,currentUID );
            }
            else
            {
                retrieve->moveSeries ( studyUID,currentUID );
            }
        }
        catch (std::exception e)
        {
    ...
    else if(currentType==static_cast<int>(ctkDICOMModel::StudyType))
    {
        logger.debug("study");
        d->setServerInformationForStudy(retrieve, currentUID);
        ...
        try
        {
            if (d->QueriesByStudyUID[currentUID]->preferCGET())
            {
                retrieve->getStudy ( currentUID );
            }
            else
            {
                retrieve->moveStudy ( currentUID );
            }
        }
        catch (std::exception e)
        {
    ...
    else if(currentType==static_cast<int>(ctkDICOMModel::PatientType))
    {
        logger.debug("patient");
        QStringList studUIDList = d->QueryResultDatabase.studiesForPatient(currentUID);
        foreach( QString studyUID, studUIDList )
        {
            if(d->UseProgressDialog){
                if (progress.wasCanceled())
                {
